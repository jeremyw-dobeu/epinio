...
func (hc Controller) GetPart(c *gin.Context) apierror.APIErrors {
	...
	if partName != "values" && partName != "chart" && partName != "image" {
		return apierror.NewBadRequest(errors.New("unknown part, expected chart, image, or values"))
	}
	...
	if app.Workload == nil {
		...
		return apierror.NewBadRequest(errors.New("No chart available for application without workload"))
	}
...
func fetchAppImage(c *gin.Context) apierror.APIErrors {
	return apierror.NewBadRequest(errors.New("image part not yet supported"))
}
